...
class StreamVGGT(nn.Module, PyTorchModelHubMixin):
    ...
    def inference(self, frames, query_points: torch.Tensor = None, past_key_values=None,
        ...
        Args:
            ...
            query_points: Optional query points.
            past_key_values: KV cache (usually no need to pass manually).
            frame_writer: Optional callback called after each frame: frame_writer(i, frame, res_cpu).
            cache_results: Whether to accumulate results in memory. Set to False for very long sequences.
        """
        ...
        for i, frame in enumerate(frames):
            ...
            # v3: Store pose to KVRepository for Pose-Aware spatial analysis
            if (hasattr(self.aggregator, 'kv_repository') and 
                self.aggregator.kv_repository is not None and
                camera_pose is not None):
                self.aggregator.kv_repository.update_pose(
                    frame_id=i + 1,
            ...
            # Invoke frame_writer callback
            if frame_writer is not None:
                frame_writer(i, frame, res_cpu)
            
            # Accumulate results based on cache_results flag
            if cache_results:
                all_ress.append(res_cpu)
                processed_frames.append(
                    {k: v.detach().cpu() if isinstance(v, torch.Tensor) else v for k, v in frame.items()}
                )
