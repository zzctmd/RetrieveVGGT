...
def main(args):
    if args.eval_dataset == "sintel":
        ...
        get_video_results()
    elif args.eval_dataset.startswith("bonn_s1_"):
        # Handle bonn_s1_50, bonn_s1_200, bonn_s1_500, etc.
        def depth_read(filename):
            # loads depth map D from png file
            # and returns it as a numpy array
            depth_png = np.asarray(Image.open(filename))
        ...
        # extract number from dataset name, e.g. bonn_s1_200 -> 200
        ...
        def get_video_results():
            ...
            gathered_depth_metrics = []
            for key in tqdm(grouped_gt_depth.keys()):
                pd_pathes = grouped_pred_depth.get(key[10:], [])
                if not pd_pathes:
                    print(f"Warning: No predictions found for {key}")
                    continue
                gt_pathes = grouped_gt_depth[key]
                gt_depth = np.stack(
                    [depth_read(gt_path) for gt_path in gt_pathes], axis=0
        ...
        # Support both short-sequence "kitti" and long-sequence "kitti_s1_xxx"
        if args.eval_dataset == "kitti":
            ...
            # extract number from dataset name, e.g. kitti_s1_200 -> 200
        ...
        def get_video_results():
            ...
            for key in tqdm(grouped_pred_depth.keys()):
                ...
                # for depth eval, set align_with_lad2=False to use median alignment; set align_with_lad2=True to use scale&shift alignment
                # KITTI: use max_depth=80 (official eval range)
                kitti_max_depth = 80
                if args.align == "scale&shift":
                    depth_results, error_map, depth_predict, depth_gt = (
                        depth_evaluation(
